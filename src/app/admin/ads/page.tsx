...
const PRESET_SAMPLE_ADS = [
  {
    name: "NITTE Admissions Open 2026",
  ...
  {
    name: "Manipal Group 2026 Admissions",
  ...
  return (
    <div className="space-y-7 font-sans text-left pb-12">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap border-b border-slate-200/80 pb-6">
        <div>
          <h1 className="font-serif text-2xl md:text-3xl font-bold text-[#0F172A] flex items-center gap-3">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-[#0F172A] hover:bg-slate-800 text-[#D4AF37] font-bold text-xs px-5 py-3 rounded-2xl shadow-sm hover:shadow-md transition duration-200 cursor-pointer border border-[#D4AF37]/40"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3.5">
        ...
        <button
          ...
          className={`p-4 rounded-2xl border text-left transition-all duration-200 cursor-pointer relative overflow-hidden flex flex-col justify-between ${
            filterPlacement === "ALL"
              ? "bg-[#0F172A] text-white border-[#0F172A] shadow-md ring-2 ring-[#D4AF37]"
              : "bg-white hover:bg-slate-50 border-slate-200 text-slate-900 shadow-2xs"
          }`}
        >
          <div className="flex items-center justify-between">
            <span className={`text-[10px] font-mono uppercase font-bold tracking-wider ${filterPlacement === "ALL" ? "text-[#D4AF37]" : "text-slate-500"}`}>
              Total Ads
            </span>
            <Layers className={`w-4 h-4 ${filterPlacement === "ALL" ? "text-[#D4AF37]" : "text-slate-400"}`} />
          </div>
          <div className="mt-3">
            <p className="text-2xl md:text-3xl font-serif font-bold">{ads.length}</p>
            <p className={`text-[10px] font-medium mt-0.5 ${filterPlacement === "ALL" ? "text-slate-300" : "text-slate-500"}`}>All Banner Campaigns</p>
          </div>
        </button>
          ...
          {[
            ...
            >
              ...
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono font-bold ${
                filterPlacement === tab.id ? "bg-[#0F172A] text-[#D4AF37]" : "bg-slate-200 text-slate-600"
              }`}>
                {tab.count}
              </span>
            </button>
      ...
      {loading ? (
        ...
        <div className="border border-dashed border-slate-300 rounded-3xl p-12 text-center max-w-md mx-auto my-8 bg-slate-50/50 space-y-4">
          ...
          <div>
            <h3 className="font-serif text-base font-bold text-[#0F172A]">No Ad Banners Found</h3>
            <p className="text-xs text-slate-500 mt-1">Create your first ad banner to display in the right sidebar!</p>
          </div>
          <button
            onClick={openCreateModal}
            className="bg-[#0F172A] text-white font-bold text-xs px-4 py-2 rounded-xl hover:bg-slate-800 transition"
          >
            Create Banner
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredAds.map((ad) => (
            <div
              key={ad.id}
              className={`bg-white border rounded-2xl p-4 shadow-xs space-y-3 transition flex flex-col justify-between ${
            ...
            >
              <div className="space-y-3">
                ...
                <div className="aspect-[3.2/1] rounded-xl overflow-hidden bg-slate-100 relative border border-slate-200">
                  ...
                  <span className="absolute top-2 left-2 text-[9px] font-mono font-bold bg-[#0F172A] text-white px-2 py-0.5 rounded uppercase shadow-xs">
                    Order #{ad.displayOrder}
                  </span>
                  <span className={`absolute top-2 right-2 text-[9px] font-mono font-bold px-2 py-0.5 rounded shadow-xs border ${
                    ad.isActive 
                ...
                {/* Info */}
                <div>
                  <h4 className="font-serif font-bold text-sm text-[#0F172A] truncate" title={ad.name}>
                    {ad.name}
                  </h4>
                  <a
                    href={ad.targetUrl}
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-lg shadow-2xl p-6 space-y-5 animate-fadeIn max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveAd} className="space-y-4 text-xs font-sans">
              <div>
                <label className="block font-bold text-slate-700 mb-1">Ad Name / Title *</label>
                <input
                  type="text"
                  required
                  placeholder="e.g. NITTE Admissions Open 2026"
                  value={form.name}
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="bg-[#0F172A] text-[#D4AF37] hover:bg-slate-800 font-bold px-5 py-2 rounded-xl transition duration-200 shadow-md"
                >
